...
| `make test` | Short unit tests: `go test -short ./... -timeout 120s`. CI (`test.yml`) runs the same suite with `-race`, which needs cgo and a C compiler locally |
...
Unit tests never reach a **Windows** host, but they do speak SSH: `internal/winclient/transport_test.go` drives real handshakes against the in-process server in `sshtest_test.go` (x/crypto's server side, loopback, no build tag, milliseconds). That tier is what covers auth method selection, host key acceptance/rejection, the `-EncodedCommand` + UTF-16LE-on-stdin bootstrap, and (since #81) persistent-session reuse, concurrent-call serialisation and recovery from a dropped session — none of which fakes can prove. Keep it free of Docker, network and external images so it stays part of the default `make test`.
...
### winclient layer
`client.go` is the SSH wrapper. The key primitive is `RunPowerShell(ctx, script)`. Only a fixed bootstrap rides on the command line via `powershell.exe -EncodedCommand`; the real script (UTF-16LE base64) is streamed on **stdin** and decoded by the bootstrap, so the command line stays a constant size and never hits Windows' ~8191-char limit regardless of script size. `RunPowerShellWithInput` sends secrets alongside the script on stdin so plaintext never appears in the encoded command or SSH session logs — use it for passwords and other sensitive values; the script reads them via `[Console]::In.ReadLine()` / `ReadToEnd()`. Since #81, a `Client` keeps **one persistent PowerShell session** open (`session.go`) and reuses it across calls instead of dialing and starting a fresh `powershell.exe` every time; calls are serialised, and a dead session is transparently re-established. This means `exit` inside a script now kills that shared session rather than just ending the call — scripts must return a status the caller checks instead, migrated resource by resource (see ADR-0010's 2026-08-28 amendment).

`auth.go` holds the two security-critical decisions, kept out of `client.go` deliberately: `buildAuthMethods` assembles the credentials to offer (explicit private key → ssh-agent → password, the conventional OpenSSH order), and `hostKeyCallback` builds the server identity check. Host key verification is **on by default** and fails closed: with neither `HostKey` (an inline pinned public key) nor a readable `known_hosts` file, `New` returns an error rather than connecting unverified. `InsecureIgnoreHostKey` is the explicit opt-out; it cannot be combined with the other two, and the provider layer raises a warning diagnostic whenever it is set. When touching this file, keep error messages free of key material and keep "host not known" distinguishable from "host key changed" — conflating them trains operators to ignore a real MITM signal.
...
This provider is generated by a multi-agent "KDust" pipeline; most files carry a "Generated by KDust" header. Pipeline prompts live in `.kdust/prompts/v2/` and design decisions in `docs/adr/` (notably **ADR-0009** for resource-vs-data-source naming/file conventions, **ADR-0010** for the SSH transport and **ADR-0011** for host key verification). When adding a resource by hand, mirror an existing one end-to-end (e.g. `feature`) so it stays consistent with what the pipeline emits. `docs/` is generated from schemas + `examples/` + `templates/` — run `make docs` and commit the result. The `docs` workflow is currently a **schema-validation gate only**: an invalid schema fails it, but the drift step is `continue-on-error: true` because only `templates/index.md.tmpl` exists and strict drift would delete the curated per-resource Markdown (see the header of `.github/workflows/docs.yml` for the migration plan). So a stale `docs/` will not fail CI: when you change a schema description by hand, mirror it into `docs/` yourself.
...
- Authentication supports public key (`private_key`, `private_key_path`, `private_key_passphrase`), ssh-agent (`use_agent`, auto-detected from `SSH_AUTH_SOCK`) and password. At least one is required; every configured method is offered to the server.
