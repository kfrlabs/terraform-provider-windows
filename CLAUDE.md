...
### winclient layer
`client.go` is the WinRM wrapper. The key primitive is `RunPowerShell(ctx, script)`, which sends the script via `powershell.exe -EncodedCommand` (UTF-16LE base64). `RunPowerShellWithInput` pipes secrets through **stdin** so plaintext never appears in the encoded command or WinRM trace logs — use it for passwords and other sensitive values.
