...
`,
	...
	if err != nil {
		return fmt.Errorf("failed to get feature info: %w; stderr: %s", err, stderr)
	}

	var info FeatureDataSourceInfo
	if err := json.Unmarshal([]byte(stdout), &info); err != nil {
		return fmt.Errorf("failed to parse feature info: %w; output: %s", err, stdout)
	}

	if !info.Exists {
		return fmt.Errorf("Windows feature %s does not exist", name)
	}
	...
	if err := d.Set("install_state", info.InstallState); err != nil {
		return fmt.Errorf("failed to set install_state: %w", err)
	}
	if err := d.Set("feature_type", info.FeatureType); err != nil {
		return fmt.Errorf("failed to set feature_type: %w", err)
	}
	if err := d.Set("path", info.Path); err != nil {
		return fmt.Errorf("failed to set path: %w", err)
	}
	if err := d.Set("sub_features", info.SubFeatures); err != nil {
		return fmt.Errorf("failed to set sub_features: %w", err)
	}
